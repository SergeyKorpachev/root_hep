...
*/
   FILE *file1 = fopen("1out_data.txt", "r"); // 1out_data.txt // // 2out_data.txt
   if ( !(long)file1 ) return;

   FILE *file2 = fopen("1out_ann.txt", "r"); // 1out_ann.txt // // 2out_ann.txt
  ...
  for ( int i = 0; i < n; i++ ) {
    fscanf( file2, "%f %E %E %E %E %E %E %E %E %E %E %E %E %E %E\n", &energy2[i], &mean2[i], &meanerror2[i], &rms2[i], &rmserror2[i], &mean_mean2[i], &rms_mean2[i], &mean_meanerror2[i], &rms_meanerror2[i], &mean_sigma2[i], &rms_sigma2[i], &mean_sigmaerror2[i], &rms_sigmaerror2[i], &mean_chi2[i], &rms_chi2[i] );
    if ( feof( file2 ) ) break;
  }
 ...
 /*
 for ( int i = 0; i < n; i++ ) {
   fscanf( file1, "%f %E %E %E %E %E %E %E %E %E %E %E\n", &energy[i], &mean[i], &meanerror[i], &rms[i], &rmserror[i], &mean_mean[i], &rms_mean[i], &mean_meanerror[i], &mean_sigma[i], &rms_sigma[i], &mean_sigmaerror[i], &mean_chi[i] );
   if ( feof( file1 ) ) break;
 }
 
  for ( int i = 0; i < n; i++ ) {
    fscanf( file2, "%f %E %E %E %E %E %E %E %E %E %E %E\n", &energy2[i], &mean2[i], &meanerror2[i], &rms2[i], &rmserror2[i], &mean_mean2[i], &rms_mean2[i], &mean_meanerror2[i], &mean_sigma2[i], &rms_sigma2[i], &mean_sigmaerror2[i], &mean_chi2[i] );
    if ( feof( file2 ) ) break;
  }
   */
...
//   bgr->SetMarkerSize(2);
   ...
   bgr->GetXaxis()->SetTitle("E_{true}, GeV");
   cgr = new TGraphErrors( n, energy2, mean_sigma2, energyerror2, rms_sigma2 );
   ...
   agr->GetYaxis()->SetRangeUser(0.0, 5.0); // -0.25, 1.0
   agr->SetTitle(fname);
...
//   bgr->SetMarkerSize(2);
   ...
   bgr->GetYaxis()->SetTitle("E_{reco}, GeV");
   bgr->GetXaxis()->SetTitle("E_{true}, GeV");
   cgr = new TGraphErrors( n, energy2, mean_mean2, energyerror2, rms_mean2 );
...
//   bgr->SetMarkerSize(2);
   ...
   sprintf( fname, "Relative energy resolution" ); // Linearity energy resolution // Absolute energy resolution // Relative energy resolution
    agr->GetYaxis()->SetRangeUser(0.0, 0.25); // -0.25, 1.0
   agr->SetTitle(fname);
...
//   lsc1->AddEntry(agr,"without ANN","pl");
   lsc1->AddEntry(bgr,"without ANN","plf");
//   lsc1->AddEntry(cgr,"without ANN","plf");
   lsc1->AddEntry(dgr,"with ANN","plf");

   TLegend *lsc2 = new TLegend(0.55,0.62,0.90,0.92);
//   lsc2->AddEntry(agr,"without ANN","pl");
   lsc2->AddEntry(bgr,"without ANN","plf");
//   lsc2->AddEntry(cgr,"without ANN","plf");
   lsc2->AddEntry(dgr,"with ANN","plf");
    
   TLegend *lsc3 = new TLegend(0.55,0.62,0.90,0.92);
//   lsc2->AddEntry(agr,"without ANN","pl");
   lsc3->AddEntry(bgr,"without ANN","plf");
//   lsc2->AddEntry(cgr,"without ANN","plf");
   lsc3->AddEntry(dgr,"with ANN","plf");
    
   TLegend *lsc4 = new TLegend(0.55,0.62,0.90,0.92);
//   lsc2->AddEntry(agr,"without ANN","pl");
   lsc4->AddEntry(bgr,"without ANN","plf");
//   lsc2->AddEntry(cgr,"without ANN","plf");
   lsc4->AddEntry(dgr,"with ANN","plf");
