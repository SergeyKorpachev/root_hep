...
void zav() {
 ...
 Int_t num = 8;
 Double_t masS[ ] = { 0, 0, 0, 0, 0, 0, 0, 0 };
 Double_t masA[ ] = { 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0 };

 Int_t bine = 100;
 Double_t mine = 0.0001;
 Double_t maxe = 3.0;
 Char_t stxt[20];

 TH1F *h1[ num ];
 for ( Int_t i = 0; i < num; i++ ) {
   sprintf( stxt,"h1%d", i + 1 );
   h1[ i ] =  new TH1F( stxt, stxt, bine, mine, maxe );
 }
...
//   TCanvas *c2 = new TCanvas("c2","c2",1000, 100, 900,700);
   ...
   plhi[0] = 1.0; plhi[1] = 5.0; plhi[2] = 1000000.0; plhi[3] = 1.0;
...
//   fitFunc = new TF1("fitFunc", "landau", mpv - sigRange*width, mpv + sigRange*width);
   ...
   for (Int_t ii = 0; ii < 1000; ii++) { 
     ...
     Double_t xx = r.Uniform( len );
     Double_t yy = r.Uniform( len );

     Double_t tt1 = 2.0 * masA[i] * masA[i] * len * len;
     Double_t tt2 = ( xx - ( 0.5 * len )  ) * ( xx - ( 0.5 * len ) );
     Double_t tt3 = ( yy - ( 0.5 * len )  ) * ( yy - ( 0.5 * len ) );
...
//   fitsnr -> Draw("same");
//   sprintf( stxt,"hhh_%d_%3.1f_%d_.jpg", i + 1, masA[i], count );
//   c1->SaveAs(stxt);
   delete h;
   delete h2;

   if ( chi2 < 2.0 ) h1[ i ] -> Fill( temp );
 ...
 for ( Int_t i = 0; i < num; i++ ) {
   h1[ i ]->Draw();
   masS[ i ] = h1[ i ]->GetMean();
   sprintf( stxt,"1%d.jpg", i + 1 );
   c1->SaveAs(stxt);
 }

 for (Int_t j = 0; j < num; j++ ) {

   cout << " mean == " << masS[ j ] << endl;  
   cout << " alpha == " << masA[ j ] << endl;  
