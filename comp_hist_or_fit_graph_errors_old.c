...
*/
   FILE *file1 = fopen("1out_data.txt", "r"); // 1out_data.txt // // 1out_ann.txt
  ...
  for ( int i = 0; i < n; i++ ) {
    ...
    linearity_error2[ i ] = sqrt( pow( meanerror[ i ], 2 ) );
    absolute_error2[ i ] = sqrt( pow( rmserror[ i ], 2 ) );

    ratio2[ i ] =  ( mean[i] - energy[i] ) / energy[i];
    ratio_error_stat2[ i ] = sqrt( pow( (meanerror[i])/(energy[i]), 2 ) + pow( (mean[i] * energyerror[i])/(energy[i] * energy[i]), 2 ) );
    ratio_error_sys2[ i ] = 0.0;//sqrt( pow( (rms_mean[i])/(energy[i]), 2 ) + pow( (mean_mean[i] * energyerror[i])/(energy[i] * energy[i]), 2 ) );
    ratio_error2[ i ] = sqrt( ( pow( ratio_error_stat[ i ], 2 ) ) + ( pow( ratio_error_sys[ i ], 2 ) ) );

    resolution2[ i ] = rms[i] / mean[i];
    resolution_error_stat2[ i ] = sqrt( pow( (rmserror[i])/(mean[i]), 2 ) + pow( (rms[i] * meanerror[i])/(mean[i] * mean[i]), 2 ) );
    resolution_error_sys2[ i ] = 0.0;//sqrt( pow( (rms_sigma[i])/(mean_mean[i]), 2 ) + pow( (mean_sigma[i] * rms_mean[i])/(mean_mean[i] * mean_mean[i]), 2 ) );
    resolution_error2[ i ] = sqrt( ( pow( resolution_error_stat[ i ], 2 ) ) + ( pow( resolution_error_sys[ i ], 2 ) ) );
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   bgr->GetXaxis()->SetTitle("E_{true}, GeV");
   cgr = new TGraphErrors( n, energy, ratio2, energyerror, ratio_error_sys2 ); // stat2 ???
...
//   bgr->SetMarkerSize(2);
   ...
   bgr->GetXaxis()->SetTitle("E_{true}, GeV");
   cgr = new TGraphErrors( n, energy, rms, energyerror, ratio_error_sys2 );//ratio_error_sys2!!!!!!!!!!!
...
//   bgr->SetMarkerSize(2);
   ...
   bgr->GetYaxis()->SetTitle("E_{reco}, GeV");
   bgr->GetXaxis()->SetTitle("E_{true}, GeV");
   cgr = new TGraphErrors( n, energy, mean, energyerror, ratio_error_sys2 );//ratio_error_sys2!!!!!!!!!!!
...
//   lsc1->AddEntry(agr,"without ANN","pl");
   lsc1->AddEntry(bgr,"data, fit","plf");
//   lsc1->AddEntry(cgr,"without ANN","plf");
   lsc1->AddEntry(dgr,"data, nofit (stat)","plf");

   TLegend *lsc2 = new TLegend(0.55,0.62,0.90,0.92);
//   lsc2->AddEntry(agr,"without ANN","pl");
   lsc2->AddEntry(bgr,"data, fit","plf");
//   lsc2->AddEntry(cgr,"without ANN","plf");
   lsc2->AddEntry(dgr,"data, nofit (stat)","plf");
    
   TLegend *lsc3 = new TLegend(0.55,0.62,0.90,0.92);
//   lsc2->AddEntry(agr,"without ANN","pl");
   lsc3->AddEntry(bgr,"data, fit","plf");
//   lsc2->AddEntry(cgr,"without ANN","plf");
   lsc3->AddEntry(dgr,"data, nofit (stat)","plf");
    
   TLegend *lsc4 = new TLegend(0.55,0.62,0.90,0.92);
//   lsc2->AddEntry(agr,"without ANN","pl");
   lsc4->AddEntry(bgr,"data, fit","plf");
//   lsc2->AddEntry(cgr,"without ANN","plf");
   lsc4->AddEntry(dgr,"data, nofit (stat)","plf");
